...
const actions = {
  async fetchTarget({ commit }, targetId) {
    commit('SET_LOADING', true)
    commit('SET_ERROR', null)
    commit('SET_TARGET', null) // 清空旧数据
    
    try {
      const response = await api.get(`/api/targets/${targetId}/`) // 注意结尾斜杠
      ...
      if (error.response) {
        ...
        if (error.response.status === 404) {
          errorInfo.message = `目标 ID ${targetId} 不存在`
          commit('SET_TARGET', null) // 明确清除数据
        }

        // 其他状态码处理示例
        else if (error.response.status === 403) {
          errorInfo.message = "无权访问此目标"
        }
